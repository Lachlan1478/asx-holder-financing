...
- A physically settled forward or collar gives the bank a relevant interest, so a bank filing its own substantial holder notice with an ISDA confirmation annexed is bank_relevant_interest; name the client counterparty as the holder.
...
- Institutional substantial holders annex securities-lending agreements (GMSLA, 'lender can recall'). That is stock lending, not client financing: record the agreement as stock_lending with no signal.
